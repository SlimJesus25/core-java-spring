 ...
 ********************************************************************************/
 ...
 public class OrchestratorService {
	 ...
	 public OrchestrationResponseDTO orchestrationFromOriginalStore(final OrchestrationFormRequestDTO orchestrationFormRequestDTO) {
		 ...
		 SystemResponseDTO publisher = searchMatchForConsumer(orchestrationFormRequestDTO, entryList);
 
		 sendInfoToEdge4CPS(orchestrationFormRequestDTO, publisher);
	 ...
	 private SystemResponseDTO searchMatchForConsumer(OrchestrationFormRequestDTO orchestrationFormRequestDTO, List<OrchestratorStore> entryList){
 
		 // 1º Verifica se existe um servico registado com o mesmo topico que este.
 
		 if(orchestrationFormRequestDTO.getCommands().containsKey("teste")) {
			 if (orchestrationFormRequestDTO.getRequesterSystem().getMetadata().containsKey("topic")) {
				 String topic = orchestrationFormRequestDTO.getRequesterSystem().getMetadata().get("topic");
				 for (OrchestratorStore orchestratorStore : entryList) {
					 SystemResponseDTO eventualPub = this.orchestratorDriver.querySystemById(orchestratorStore.getProviderSystemId());
					 String fTopic = eventualPub.getMetadata().get("topic");
					 if(fTopic.compareTo(topic) == 0)
						 return eventualPub;
				 }
			 }
		 }
	 ...
	 private void sendInfoToEdge4CPS(OrchestrationFormRequestDTO orchestrationFormRequestDTO, SystemResponseDTO publisher){
		 if(publisher == null)
			 return;
 
		 ServiceRegistryListResponseDTO services = this.orchestratorDriver.queryAllServices();
 
		 String edgeAddress = "";
		 for(ServiceRegistryResponseDTO service : services.getData()){
			 if(service.getServiceDefinition().getServiceDefinition().equalsIgnoreCase("edge4cps")){
				 edgeAddress += service.getMetadata().get("address");
				 break;
			 }
		 }
 
		 this.orchestratorDriver.sendInformationEdge4CPS(edgeAddress);
